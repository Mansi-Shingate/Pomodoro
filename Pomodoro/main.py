from tkinter import *
import math

# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None


# ---------------------------- TIMER RESET ------------------------------- #
def reset_timer():
    window.after_cancel(timer)
    label_tick.config(text='')
    label_timer.config(text="Timer", font=(FONT_NAME, 45, "bold"), fg=GREEN, bg=YELLOW)
    canvas.itemconfig(timer_txt, text="00:00")


# ---------------------------- TIMER MECHANISM ------------------------------- #
def start_timer():
    global reps
    reps += 1
    work_secs = WORK_MIN * 60
    short_brk_secs = SHORT_BREAK_MIN * 60
    long_brk_secs = LONG_BREAK_MIN * 60

    if reps == 8:
        count_down(long_brk_secs)
        label_timer.config(text="Break", fg=RED)
    elif reps % 2 == 0:
        count_down(short_brk_secs)
        label_timer.config(text="Break", fg=PINK)
    else:
        count_down(work_secs)
        label_timer.config(text="WORK", fg=GREEN)


# ---------------------------- COUNTDOWN MECHANISM ------------------------------- #
def count_down(count):
    count_min = math.floor(count / 60)
    count_sec = count % 60
    if count_sec < 10:
        count_sec = f"0{count_sec}"
    canvas.itemconfig(timer_txt, text=f"{count_min}:{count_sec}")
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count - 1)
    else:
        start_timer()
        marks = ""
        work_sessions = math.floor(reps / 2)
        for _ in range(work_sessions):
            marks += 1
        label_tick.config(text=marks)


# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
window.title("Pomodoro")
window.config(padx=100, pady=50, bg=YELLOW)
canvas = Canvas(width=200, height=224, bg=YELLOW)
tomato_img = PhotoImage(file="tomato.png")

label_timer = Label(text="Timer", font=(FONT_NAME, 45, "bold"), fg=GREEN, bg=YELLOW)
label_timer.grid(column=1, row=0)

canvas.create_image(102, 112, image=tomato_img)
timer_txt = canvas.create_text(102, 130, text="00:00", fill="white", font=(FONT_NAME, 35, "bold"))
canvas.grid(column=1, row=1)

label_tick = Label(font=(FONT_NAME, 25, "bold"), fg=GREEN, bg=YELLOW)
label_tick.grid(column=1, row=3)

button_start = Button(text="Start", font=(FONT_NAME, 13, "bold"), command=start_timer)
button_start.grid(column=0, row=2)

button_reset = Button(text="Reset", font=(FONT_NAME, 13, "bold"), command=reset_timer)
button_reset.grid(column=2, row=2)

window.mainloop()
